#Percentage of delayed flights due to weather at departure airport
WeatherDataset <- c()
Airports <- as.character(unique(Year$Origin))
i <- 1
#Format the Year dataframe to include posix date and time
Year$CRSDepTime <- substr(as.POSIXct(sprintf("%04.0f", Year$CRSDepTime), format='%H%M'), 12, 16)
Year$Date <- paste(Year$Year,Year$Month,Year$DayofMonth, sep = "-")
Year$DateTime <- as.POSIXct(paste(as.character(Year$Date),as.character(Year$CRSDepTime)), format="%Y-%m-%d %H:%M")
#While loop to go get the data for each airport
while (i <= length(Airports)){
  #Download weather data
  Temp <- Year[Year$Origin == Airports[i],]
...
source("DepartureTemperature.R")
source("DepartureVisability.R")
source("DepartureWindSpeed.R")
rm(WeatherDataset)